...
class OrderRepository:
    ...
    def list_by_buyer(self, buyer_party_id: int, status: Optional[OrderStatus | str] = None, limit: int = 100, offset: int = 0) -> List[OrderData]:
        ...
        if status:
            status = OrderStatus(str(status)).value
    ...
    def create(self, entity: OrderData) -> OrderData:
        try:
            rec = OrderModel.create(
                ...
                flow=(entity.flow.value if entity.flow else None),
                provider=(entity.provider.value if entity.provider else None),
    ...
    def update(self, entity: OrderData) -> Optional[OrderData]:
        try:
            ...
            rec.status = entity.status.value
            rec.description = entity.description
            rec.metadata = OrderModel.dumps_metadata(entity.metadata)
            rec.flow = (entity.flow.value if entity.flow else None)
            rec.provider = (entity.provider.value if entity.provider else None)
    ...
    def set_checkout_context(
        ...
        try:
            ...
            rec.flow = PaymentFlow(str(flow)).value
            rec.provider = ProviderEnum(str(provider)).value
            rec.env = EnvEnum(str(env)).value
